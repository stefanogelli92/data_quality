...
class ValuesOrderDimensionTable(Check):
    ...
    def _create_negative_filter(self):
        cast_left_column = self.table.source.cast_float_sql("left_table." + self.left_column)
        cast_right_column = self.table.source.cast_float_sql("right_table." + self.right_column)
        sql_filter = f"coalesce({cast_left_column} {self.operator} {cast_right_column}, false)"
        return sql_filter
